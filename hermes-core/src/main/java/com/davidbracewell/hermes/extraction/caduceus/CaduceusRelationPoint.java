...
public class CaduceusRelationPoint {
   ...
   static CaduceusRelationPoint fromMap(Map<String, Object> map) throws IOException {
      ...
      if (map.containsKey("relation")) {
         ...
         if (parts.size() != 2) {
            throw new IOException("A source or target relation does not include relation name & value: " + map.get(
               "relation"));
         }
   ...
   private Stream<Annotation> getAnnotationStream(Multimap<String, Annotation> groups, TokenMatcher matcher) {
      if (groups.containsKey(captureGroup)) {
         return groups.get(captureGroup)
                      .stream();
      }
      ...
      return getAnnotationStream(groups, matcher)
                .flatMap(a -> {
                            if (relationType.equals(Types.DEPENDENCY)) {
                               return a.children().stream()
                                       .filter(a2 -> a2.dependencyRelation().v1.equals(relationValue));
                            }
