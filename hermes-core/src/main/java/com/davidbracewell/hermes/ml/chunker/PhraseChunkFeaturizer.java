...
public class PhraseChunkFeaturizer implements SequenceFeaturizer<Annotation> {
  ...
  public Set<Feature> apply(ContextualIterator<Annotation> itr) {
    ...
    if (StringUtils.isUpperCase(p0W)) {
      features.add(Feature.TRUE("isAllUpperCase"));
    } else if (Character.isUpperCase(p0W.charAt(0))) {
      features.add(Feature.TRUE("startUpperCase"));
    }
